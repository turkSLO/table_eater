    ...
    HTML("
      $(document).ready(function(){
        ...
        if(localStorage.firstTime !== 'false') {
           ...
           firstTime.next($('.col-sm-4'), 'Please insert:</br><input type=\"text\" style=\"width:575px; color:black;\" value=\"https://en.wikipedia.org/wiki/List_of_current_United_States_Senators\" /></br>into the bar on the left and press Submit.', 'right');
        }
        ...
        $('#submitURL').click(function(){
          if(localStorage.firstTime !== 'false') {
            firstTime.next($('.col-sm-8'), 'On the right will populate the tables located within that page.</br><button id=\"continue\" class=\"btn btn-default\" type=\"button\">Continue</button>', 'left');
          }
        });
        $('#guide-content').on('click','#continue',function(){
         firstTime.next($('.col-sm-4'), 'Using the arrow buttons above, you can preview the tables within the page.</br>With the empty bar below, you can select which tables you wish to download.</br>After selecting one or more tables, more buttons appear:</br>You can choose to invert the selection (pick the opposite of what you had selected).</br>You can choose to download the tables in CSV format.</br>You can choose to download the tables in TXT format.</br></br>This is the end of the tutorial. To close, hit the button on the bottom right.', 'bottom');
